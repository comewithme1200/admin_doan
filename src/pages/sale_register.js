...
const SaleRegister = () => {
  ...
  const [productsCart, setMoviesCart] = useState([]);
  const { clients, loguedUser, products, setClients, setMovies, isAdmin, setLoguedUser, setIsAdmin } = useContext(AppContext);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (!products) {
        const { data, request } = await getMovies(token, null);
